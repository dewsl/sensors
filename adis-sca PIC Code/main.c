...
// CONFIG2H
#pragma config WDTEN = OFF  		// Watchdog Timer (WDT disabled)
...
volatile unsigned char temp,xh1=0,xl1=0,yh1=0,yl1=0,zh1=0,zl1=0,xh2=0,xl2=0,yh2=0,yl2=0,zh2=0,zl2=0;	// Accelerometer raw variables
...
/*
    ...
    get_somsraw(0);
   	if (DEBUG) {sprintf(string,(const char *)"S IRaw: %d\n\r", soms_raw); puts1USART(string); }
   	get_somscal();
   	if (DEBUG) {sprintf(string,(const char *)"S ICal: %d\n\r", soms_cal); puts1USART(string); }
*/
    while(1)
    {
...
void get_axel1raw (void)		//SCA
{	
	//axel_initialize();
	
	//PORTC &= ~0x01;				// Assert chip select
/*	
	temp = (unsigned char)putcSPI(X_MSB);	// Send low byte of address
	xh1 = getcSPI();
	temp = (unsigned char)putcSPI(X_LSB);	// Send low byte of address
	xl1 = getcSPI();
    temp = (unsigned char)putcSPI(Z_MSB);	
	zh1 = getcSPI();
    temp = (unsigned char)putcSPI(Z_LSB);	// Send low byte of address
	zl1 = getcSPI();
    temp = (unsigned char)putcSPI(Y_MSB);
	yh1 = getcSPI();
    temp = (unsigned char)putcSPI(Y_LSB);	// Read single byte
	yl1 = getcSPI();			// Read single byte
*/	
	xl1 = axel_readbyte(X_LSB);	// read x
	xh1 = axel_readbyte(X_MSB);
	yl1 = axel_readbyte(Y_LSB);	// read y
	yh1 = axel_readbyte(Y_MSB);
	zl1 = axel_readbyte(Z_LSB);	// read z
	zh1 = axel_readbyte(Z_MSB);
...
/*
	temp = (unsigned char)putcSPI(XACCL_OUT);	// Send low byte of address
	temp = (unsigned char)putcSPI(0x00);
	
    xh2 = getcSPI();			// Read single byte
	xl2 = getcSPI();			// Read single byte
    
	temp = (unsigned char)putcSPI(YACCL_OUT);	// Send low byte of address
	temp = (unsigned char)putcSPI(0x00);	
	
    yh2 = getcSPI();			// Read single byte
	yl2 = getcSPI();			// Read single byte
    
    temp = (unsigned char)putcSPI(ZACCL_OUT);	// Send low byte of address
	temp = (unsigned char)putcSPI(0x00);
	zh2 = getcSPI();			// Read single byte
	zl2 = getcSPI();			// Read single byte
*/

	xval = axel2_readbyte(XACCL_OUT);	// read x
    yval = axel2_readbyte(YACCL_OUT);	// read x
    zval = axel2_readbyte(ZACCL_OUT);	// read x
    
    
    xl2 = (unsigned char)((int)xval & 0xFF);
    xh2 = (unsigned char)((int)xval >> 8);
    yl2 = (unsigned char)((int)yval & 0xFF);
    yh2 = (unsigned char)((int)yval >> 8);
    zl2 = (unsigned char)((int)zval & 0xFF);
    zh2 = (unsigned char)((int)zval >> 8);
}
...
*/
	...
	xval = axel2_readbyte(XINCL_OUT);	// read x
    yval = axel2_readbyte(YINCL_OUT);	// read y
    zval = axel2_readbyte(ZINCL_OUT);	// read z
...
{
	PORTC |= 0x01;
    //OpenSPI (SPI_FOSC_16, MODE_00, SMPEND);	//SCA
...
{
	PORTB |= 0x01;
    //OpenSPI (SPI_FOSC_16, MODE_11, SMPEND);	//ADIS
...
int axel2_readbyte (unsigned char address)
{
    ...
    unsigned int msb, lsb,temp;
    int val;
    unsigned char *rdptr; 
    
	OpenSPI (SPI_FOSC_16, MODE_11, SMPEND);	//ADIS
	//temp=transfer(address);
    
    
    //address |= 0x80;			// Set to 1 the MSB for read
	PORTB &= ~0x01;				// Assert chip select
    Delay1KTCYx(32);//delay 160ms
    
    temp = putcSPI(address);	// Send MSB of address
    temp = putcSPI(0x00);    // Send LSB of address
    
    Delay1KTCYx(20);//delay 100ms
    //msb = getcSPI();			// Read MSB
    //putcSPI(0x00);    //send dummy byte
    //lsb = getcSPI();
    //getsSPI(rdptr, 2); //read multiple bytes
    msb = getcSPI();//(unsigned int)rdptr[0];
    lsb = getcSPI();//(unsigned int)rdptr[1];

    val = (msb << 8) | lsb;			// Read LSB byte and join with MSB

    PORTB |= 0x01;				// Negate chip select
	
    Delay1KTCYx(10);//delay 50ms
    CloseSPI();
    return (val);				// Return read data
}
...
{	
	...
	soms_raw = (unsigned int)get_median(soilbuffer, SOMSSAMPLES);
}
...
//bubblesort from <http://www.tigoe.com/pcomp/code/arduinowiring/42/>
    ...
    for(out=0 ; out < numReadings; out++) {  // outer loop
              for(in=out; in<(numReadings-1); in++)  {  // inner loop
                          ...
                          // swap them:
                                  ...
                                  analogValues[in+1] = (unsigned char)swapper;
                          }
              }
    }
	...
	somsadc = 0;
 	...
 	for(c = 12; c<37; c++)	sum = sum + analogValues[c]; 	// get sum of 25 middle samples
 	average = (unsigned int)(sum/(SOMSSAMPLES/2))*bg*2/4096;			//get average and multiply with voltage
	somsadc = (unsigned int)(sum/25);
 	soil = (int)(average);
	if (DEBUG) sprintf(string,(const char *)"SOMS: %d,%d \n\r", soil,somsadc); puts1USART(string);
...
{	
	...
	somsair = (unsigned int)((somsairh*256) + somsairl);
	somswater = (unsigned int)((somswaterh*256) + somswaterl);
