...
void main(void)
{
    ...
    while(1)
    {
         ...
         if(status)	command = can_process_commands_extended(&canBuffer,unique_nodeid);
         ...
         switch(command) {
             ...
             case 99:
   				...
   				if (DEBUG) {sprintf(string,(const far rom char *)"COMMAND 99\n\r"); puts1USART(string); }
         ...
         if((status !=0) && (command != 0))	{ //check if received something
                 ...
                 if (DEBUG) { sprintf(string,(const far rom char *)"  TX..."); puts1USART(string); }
                 if (DEBUG) { sprintf(string,(const far rom char *)"  %d: %d_%d_%d_%d_%d_%d_%d_%d\n\r",
...
//bubblesort from <http://www.tigoe.com/pcomp/code/arduinowiring/42/>
	...
	somsadc = 0;
 	...
 	average = (sum/(SOMSSAMPLES/2))*bg*2/4096;			//get average and multiply with voltage
	somsadc = (int)(sum/25);
 	soil = (int)(average);
	if (DEBUG) sprintf(string,(const far rom char *)"SOMS: %d,%d \n\r", soil,somsadc); puts1USART(string);
