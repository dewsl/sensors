...
bagong function
*/
float * get_data_polling(int cmd, int uid){
  int retry_count = 0,respondents = 0;
  int count,i=0,c;
  int x,y,z;
  ...
  CAN_FRAME incoming;
//  if (cmd < 100){
    //Serial.println(count_frames(g_can_buffer)-1);
  turn_on_column();
  poll_command(cmd,uid); //hindi ko ba kailangang ireset yung column?
      //Serial.print(F("Polling UID: "));
      //Serial.print(uid);
  ...
  do {
    ...
    }while ((millis() - timestart <= POLL_TIMEOUT));
...
void interpret_frame(CAN_FRAME incoming){
  ...
  if ((d1 == 8)|(d1 == 9)|(d1 == 11)|(d1 == 12)|(d1==32)|(d1==33)|(d1 == 41)|(d1 == 42)|(d1==50)|(d1 == 17)|(d1==19)|(d1==20)|(d1==16)|(d1==51)|(d1==52)){ 

    if ((d1 == 8)|(d1 == 11)|(d1==32)|(d1==41)|(d1==17)|(d1==51)){
      axel = 1;
    }
    else if ((d1 == 9)|(d1 == 12)|(d1==33)|(d1==42)|(d1==19)|(d1==52)){
      axel = 2;
    }
    else {axel=d1; } //msgid
...
//    Serial.print(id,HEX); 
    ...
    Serial.print(axel); Serial.print(',');
//    Serial.print(convert_uid_to_gid(id)); Serial.print('\t');
    ...
    if ((d1 == 11)|(d1 == 12)|(d1 == 41)|(d1 == 42)|(d1 == 51)|(d1 == 52)){Serial.print(" ");Serial.print(float(d8+200)/100.0);}
    Serial.println("");
...
void write_eeprom(int command, int uid,int transid, int data1, int data2){
  int i=0,d1,d2,d3,d4,d5,d6,d7,d8;
  int timestart = millis();
  if (VERBOSE == 1) { Serial.println("send_frame()"); }
  
  CAN_FRAME outgoing, incoming;
  //CAN_FRAME incoming;
  turn_on_column();
  ...
  outgoing.data.byte[1] = uid >> 8;
  outgoing.data.byte[2] = uid & 0xFF;
  outgoing.data.byte[3] = transid;
  outgoing.data.byte[4] = data1 & 0xFF; //low bit
  outgoing.data.byte[5] = data1 >> 8;   //high bit
  outgoing.data.byte[6] = data2 & 0xFF;
  outgoing.data.byte[7] = data2 >> 8;
  Can0.sendFrame(outgoing);
    ...
    if (Can0.available()){
      ...
      Serial.print("--");
      Serial.print(incoming.id);              Serial.print(", ");
      Serial.print(incoming.data.byte[0]);    Serial.print(", ");
      Serial.print(incoming.data.byte[1]);    Serial.print(", ");
      Serial.print(incoming.data.byte[2]);    
      if (command %100 == 35){Serial.println();}
      else{
        Serial.print(", "); Serial.print(incoming.data.byte[3]);    
        Serial.print(", "); Serial.print(incoming.data.byte[4]);
        if (command %100 == 21){Serial.println();}    
        else{Serial.print(", "); Serial.println(incoming.data.byte[5]);}
      }
      if (command > 100 && incoming.id == uid) {break;}

      }
      
    }while ((millis() - timestart <= broad_timeout));
    turn_off_column();
}


void write_calib_param(){
  int ind=0, temp = -1, cmd;
  int *parsed_data = (int*)malloc(sizeof(int)*14);
  String readstr, tempstr;
      ...
      Serial.println(readstr); //prints string to serial port

      for (int i=0; i<14; i++){
        ind = readstr.indexOf('#',temp+1);
        tempstr = readstr.substring(temp+1, ind);
        parsed_data[i] = tempstr.toInt();
      ...
      write_eeprom(cmd, parsed_data[0],1,parsed_data[2],parsed_data[3]);
      write_eeprom(cmd, parsed_data[0],2,parsed_data[4],parsed_data[5]);
      write_eeprom(cmd, parsed_data[0],3,parsed_data[6],parsed_data[7]);
      write_eeprom(cmd, parsed_data[0],4,parsed_data[8],parsed_data[9]);
      write_eeprom(cmd, parsed_data[0],5,parsed_data[10],parsed_data[11]);
      write_eeprom(cmd, parsed_data[0],6,parsed_data[12],parsed_data[13]);
